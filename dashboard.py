...
class Dashboard:
    ...
    def _build_layout(self) -> Layout:
        ...
        layout.split_column(
            ...
            Layout(self._accounts_panel(),    size=6),
            Layout(self._stats_panel(),       size=6),
            Layout(self._recent_executions(), size=12),
        )
        return layout
    ...
    def _accounts_panel(self) -> Panel:
        ...
        for a in accounts:
            ...
            style  = "green" if status == "ALIVE" else "red" if status == "STOPPED" else "yellow"
            t.add_row(
    ...
    def _stats_panel(self) -> Panel:
        ...
        t.add_row(
            "Skipped Executions",
            f"[yellow]{stats.get('skipped_executions', 0)}[/yellow]",
        )
        t.add_row(
            "Failed Executions",
    ...
    def _recent_executions(self) -> Panel:
        rows = self.db.get_recent_executions(limit=10)
        t    = Table(box=box.SIMPLE_HEAVY, expand=True, show_lines=False, padding=(0,1))
        t.add_column("Time",          style="dim", width=19)
        t.add_column("Slave",         width=10)
        t.add_column("Master Ticket", width=13)
        t.add_column("Symbol",        width=8)
        t.add_column("Action",        width=14)
        t.add_column("Status",        width=8)
        t.add_column("RC",            width=5)
        t.add_column("Latency",       width=8)
        ...
        return Panel(
            t, title="[bold]Recent Executions[/bold]", border_style="blue"
        )